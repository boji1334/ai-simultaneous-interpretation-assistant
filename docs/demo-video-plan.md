...
6. Show metrics: first subtitle latency, correction latency, glossary hit rate, final stability rate.
7. Show glossary: explain why `attention mechanism` maps to `注意力机制`.
8. Click `生成总结`: show summary, key points, terms, and correction notes.
9. Click `下载 MD` or `下载 SRT`: show export capability.
10. Click `加载最终字幕`: explain this is a fallback snapshot for stable review/demo.
11. Closing: mention tests, CI, PR template, original boundaries, and self-written demo material.
